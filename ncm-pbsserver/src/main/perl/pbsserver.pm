...
sub Configure
{
    ...
    if ( $pbsserver_config->{confdir} ) {
      $confdir = $pbsserver_config->{confdir};
    }
    if ( $pbsserver_config->{logdir} ) {
      $logdir = $pbsserver_config->{logdir};
    }

    for my $p ($pbsroot, $confdir, $logdir) {
      mkpath($p, 0, 0755) unless (-e $p);
      if (! -d $p) {
        $self->Fail("Can't create directory: $p");
        return 1;
      }
    }
    ...
    if ( $pbsserver_config->{env} ) {
        my $fname = "$confdir/pbs_environment";
    ...
    ## this is far from true. if something else manages torque.cfg, set ignoretorquecfg to true
        ...
        unlink "$confdir/torque.cfg" if (-e "$confdir/torque.cfg" && $removetorquecfg);
